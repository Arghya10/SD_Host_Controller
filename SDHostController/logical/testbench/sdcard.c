...
uint8_t dataBlock0[1024], dataBlock1[1024], dataBlock2[1024], dataBlock3[1024],readStop3 =0;
...
void DAT1()
{
        ...
        for(i=0;i<1024;i++)
        {
                dataBlock1[i] = dataBlock1new[i+1];
        }
        readStop = dataBlock1new[1041];
}
void DAT2()
{
        uint8_t readStart =1, readStop;
       ...
       read_uint8_n("DAT2Tx",dataBlock2new,1042);  
        ...
        for(i=0;i<1024;i++)
        {
                dataBlock2[i] = dataBlock2new[i+1];
        }
        readStop = dataBlock2new[1041];
}
void DAT3()
{
        uint8_t readStart =1;//readStop;
        ...
        readStart = dataBlock3new[0];
        for(i=0;i<1024;i++)
        {
                dataBlock3[i] = dataBlock3new[i+1];
        }
        readStop3 = dataBlock3new[1041];
}
DEFINE_THREAD(DAT0);
DEFINE_THREAD(DAT1);
DEFINE_THREAD(DAT2);
       ...
       //dataBlocks = malloc(100 * sizeof(uint8_t *));
        ...
        case RESP_R1b:
                ...
                for(i=0;i<48;i++)
                {
                        ...
                        //write_uint8("responseRx",(responsebit << 7));
                        responsearray[i] = responsebit;
        ...
        case RESP_R2S:
                ...
                for(i=0;i<136;i++)
                {
                        ...
                        if((i==0)||(i==1))
                        {
                                ...
                                bigresponsearray[i] = 0x0;
                                //fprintf(stderr,"0");
                        }
                        else if ((i>=2)&&(i<=7))
                        {
                                //write_uint8("responseRx",(0x80));
                                bigresponsearray[i] = 0x1;
                                //fprintf(stderr,"1");
                        }
                        else if((i>=8)&&(i<=71))
                        {
                                ...
                                //write_uint8("responseRx",(responsebit<<7));
                                bigresponsearray[i] = responsebit;
                                //fprintf(stderr,"%x",responsebit);
                                data[0] = data[0] << 1U;
                        }
                        else if((i>=72)&&(i<=134))
                ...
                write_uint8_n("responseRx",bigresponsearray,136);
                //fprintf(stderr,"\n");
                break;
        case RESP_R3:
                temp_ocr = sdCard.ocr;
        ...
        if(commandIndex == 0x110000000000)
        {
              ...
              if(busWidth == 0)
              {
                ...
                for(i=0;i<4096;i++)
                {
                         write_uint8("DAT0Rx",(dataBlock[i]));
                }
                crc = CRC_16(dataBlock,4096);
                for(i=0;i<16;i++)
                {
                         write_uint8("DAT0Rx",(crc[i]));
                }
              ...
              else
              {
                       ...
                       //fprintf(stderr,"\nStarts\n\n");
                        ...
                        for(i=0;i<16;i++)
                        {
                                ...
                                DAT0Rxarray[i+1025] = (crc[i]);
                        }
                        DAT0Rxarray[1041]=0x1;
                        //write_uint8("DAT0Rx",0x80);
                        ...
                        for(i=0;i<16;i++)
                        {
                                ...
                                DAT1Rxarray[i+1025] = (crc[i]);
                        }
                        //write_uint8("DAT1Rx",0x80);
                        DAT1Rxarray[1041] = 0x1;
                        ...
                        for(i=0;i<16;i++)
                        {
                                ...
                                DAT2Rxarray[i+1025] = (crc[i]);
                        }
                        //write_uint8("DAT2Rx",0x80);
                        DAT2Rxarray[1041] = 0x1;
                        DAT3Rxarray[0] = 0x0;
                        ...
                        for(i=0;i<16;i++)
                        {
                                ...
                                DAT3Rxarray[i+1025] = (crc[i]);
                        }
                        //write_uint8("DAT3Rx",0x80);  
                        DAT3Rxarray[1041] = 0x1;
                        PTHREAD_DECL(DAT0R);
                        PTHREAD_CREATE(DAT0R);
                        PTHREAD_DECL(DAT1R);
                        PTHREAD_CREATE(DAT1R);
                        PTHREAD_DECL(DAT2R);
        ...
        if(commandIndex == 0x120000000000)
        {
              ...
              if(busWidth == 0)
              {
                ...
                for(i=0;i<4096;i++)
                {
                        write_uint8("DAT0Rx",(dataBlock[i] << 7));
                }
                crc = CRC_16(dataBlock,4096);
                //write_uint8_n("DAT0Rx",crc,16);
                for(i=0;i<16;i++)
