...
{ 
        ...
        sdclkFreq = 0x7D;//SD clock frequency set to 400KHz (the default frequency)
	data =(sdclkFreq << 8) | 1 ;
        ...
        fprintf(stderr,"HOST: CLock Control = %x\n",readData);
        ...
        fprintf(stderr,"HOST: CLock Control = %x\n",readData);
        //SD clock Enable
        ack = ReadWriteSDHCRegister(0,2,(SDBase + ClockControl),0x7D05);
        ...
        sdclkFreq = 1;//SD clock frequency set to 25MHz frequency 
	data =(sdclkFreq << 8) | 1 ;
        ack = ReadWriteSDHCRegister(0,2,(SDBase + ClockControl),data);//Internal clock enable and SDCLK frequency select
        readData = ReadWriteSDHCRegister(1,2,(SDBase + ClockControl),0);
        fprintf(stderr,"HOST: CLock Control = %x\n",readData);
        ...
        fprintf(stderr,"HOST: CLock Control = %x\n",readData);
        //SD clock Enable
        ack = ReadWriteSDHCRegister(0,2,(SDBase + ClockControl),0x105);
